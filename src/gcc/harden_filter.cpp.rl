...
%%{
	...
	pi = ( flag_prefix pi_suffix  | '-' maybe_no 'pie' ) %{ if (env.position_independent) return false; };
	nso = flag_prefix 'strict-overflow' %{ if (env.no_strict_overflow) return false; };
	action fsa { if (env.fortify_source) return false; }
	fs = /-[UD]_FORTIFY_SOURCE/ %fsa | /-D_FORTIFY_SOURCE=/ @fsa;
	sp = flag_prefix 'stack-protector' @{ if (env.stack_protector) return false; };
